...
export function parsePlanFromTranscript(planContent: string): ParsedPlan | null {
  const titleMatch = planContent.match(/^#\s*(?:計画|Plan):\s*(.+)$/m);
  if (!titleMatch) {
    return null;
  }
  ...
  const titleIndex = planContent.indexOf(titleMatch[0]);
  let body = planContent.slice(titleIndex + titleMatch[0].length).trim();
